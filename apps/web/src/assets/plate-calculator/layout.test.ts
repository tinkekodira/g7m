...
describe('layoutBar', () => {
  it('places the collar against the stop on an empty bar', () => {
    ...
    // plate_start, in world coordinates: -(611.16 + (459.28 - 432.33))
    expect(layout.collar.x + 43.44).toBeCloseTo(-638.11, 1);
  });
  ...
  it('stacks plates outward from the stop, each butting the one before it', () => {
    ...
    expect((red?.x ?? 0) + 135.57).toBeCloseTo(-638.11, 1);
    // The blue plate's anchor sits one red-thickness further out.
    expect((blue?.x ?? 0) + 122.72).toBeCloseTo(-638.11 - 73.15, 1);
    // The collar follows the blue plate by blue's own advance.
    expect(layout.collar.x + 43.44).toBeCloseTo(-638.11 - 73.15 - 65.45, 1);
  });
  ...
  it('sets the frame half-width to the sleeve tip, independent of what is loaded', () => {
    ...
    expect(empty.halfWidth).toBeCloseTo(1058.7, 1); // coincidentally unchanged from the old bar
