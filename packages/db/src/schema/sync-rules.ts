...
#              receives only rows it owns. PowerSync replicates through a
#              logical replication slot, which bypasses row level security
#              entirely — so this parameter is not an optimisation, it is the
#              access control. A mistake here publishes one user's training
#              history to another.
